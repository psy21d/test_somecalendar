<template>
  <VRangeSelector
    ...
    @update:start-date="(date) => $emit('update:start-date', formatDate(date))"
    @update:end-date="(date) => $emit('update:end-date', formatDate(date))"
  />
</template>
...
import VRangeSelector from './components/RangeSelector.vue'
...
export default class Calendar extends Vue {
  @Prop({ default: null }) startDate!: string
  @Prop({ default: null }) endDate!: string
  ...
  parseDate(date: string) {
    let result = dayjs(date, 'DD.MM.YYYY').format('YYYY-MM-DD')
    if (result === 'Invalid Date') {
      return null
    }
    return result
